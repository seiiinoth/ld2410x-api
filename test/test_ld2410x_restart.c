...
void test_ld2410x_restart()
{
  ...
  uint8_t exp_tx[] = {0xFD, 0xFC, 0xFB, 0xFA, 0x02, 0x00, 0xA3, 0x00, 0x04, 0x03, 0x02, 0x01};
  uint8_t exp_rx[] = {0xFD, 0xFC, 0xFB, 0xFA, 0x04, 0x00, 0xA3, 0x01, 0x00, 0x00, 0x04, 0x03, 0x02, 0x01};
  ...
  uint8_t rslt = ld2410x_restart(&dev);
...
/* ERROR TEST */
/* Failure ACK status */
void test_ld2410x_restart_fail_ack()
{
  struct ld2410x_dev dev = ld2410x_dev_init_zero;
  ...
  uint8_t exp_rx_fail[] = {0xFD, 0xFC, 0xFB, 0xFA, 0x04, 0x00, 0xA3, 0x01, 0x01, 0x00, 0x04, 0x03, 0x02, 0x01};
...
int main(void)
{
  ...
  RUN_TEST(test_ld2410x_restart);
  RUN_TEST(test_ld2410x_restart_fail_ack);
